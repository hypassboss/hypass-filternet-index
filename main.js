...
const carData = { 
  ...
  "Mitsubishi": ["Outlander", "Eclipse Cross", "Colt Plus", "其他"], "Hyundai": ["Tucson", "Custin", "Venue", "其他"],
  "Kia": ["Sportage", "Sorento", "EV6", "其他"], "Volkswagen": ["Golf", "Tiguan", "其他"], "Skoda": ["Kodiaq", "Kamiq", "其他"],
  "Lexus": ["NX", "RX", "UX", "ES", "其他"], "Benz": ["C-Class", "E-Class", "GLC", "GLE", "其他"],
  "BMW": ["3-Series", "5-Series", "X3", "X5", "其他"], "Audi": ["A3", "A4", "Q3", "Q5", "其他"],
  "Volvo": ["XC60", "XC40", "其他"], "Porsche": ["Macan", "Cayenne", "其他"], "Tesla": ["Model Y", "Model 3", "Model X", "Model S"],
  "Subaru": ["Forester", "XV", "其他"], "Suzuki": ["Swift", "Jimny", "其他"], "Luxgen": ["URX", "n7", "其他"], "Other": ["其他品牌"] 
};
...
function updateCarModels(bId, mId) { 
    const b = document.getElementById(bId).value; const m = document.getElementById(mId); m.innerHTML = '<option value="">* 選擇車型</option>'; 
    if(carData[b]) carData[b].forEach(i => m.innerHTML+=`<option value="${i}">${i}</option>`); 
}

function updateDistricts(cityId, distId) { 
    const c = document.getElementById(cityId).value; const d = document.getElementById(distId); d.innerHTML = '<option value="">* 鄉鎮市區</option>'; 
...
async function submitRegister(role) {
    try {
        ...
        const payload = { 
            ...
            license_plate: pl, yearly_mileage: parseInt(mil) || 10000
        };
...
async function shareToLine() {
    ...
    if (liff.isApiAvailable('shareTargetPicker')) { 
        try { await liff.shareTargetPicker([{ type: "text", text: `推薦您加入 HYPASS 智能座艙：\n${link}` }]); } catch (e) {} 
    } else { navigator.clipboard.writeText(link); alert(`請複製並傳送給好友：\n${link}`); }
}
  ...
  if (data && data.length > 0) {
    ...
    data.forEach(g => { 
        html += `<div class="g-card" style="text-align:left;">
            ...
            <button class="btn-main" style="padding:10px; font-size:14px; margin-top:5px;" onclick="bookGarage(${g.id})">預約此廠</button>
        </div>`; 
    });
  } else { html = '目前無合作保修廠'; } 
...
async function bookGarage(gId) { 
    let d = prompt("輸入希望預約的日期 (ex: 2026/03/01):"); 
    if(d) { await supabaseClient.from('bookings').insert([{user_uid: currentUser.line_uid, garage_id: gId, book_date: d}]); alert("✅ 預約已送出"); } 
}
...
async function loadBulletins() {
  ...
  if (!error && data && data.length > 0) {
      ...
      data.forEach(b => {
          html += `<div class="g-card" style="padding:15px; border-left:4px solid var(--accent-color); text-align:left;">
                      <div style="font-size:11px; color:var(--text-secondary); margin-bottom:5px;">${new Date(b.created_at).toLocaleDateString()}</div>
                      <div style="font-weight:bold; font-size:15px; color:var(--text-primary); margin-bottom:6px;">${b.title}</div>
                      <div style="font-size:13px; color:var(--text-secondary); line-height:1.5; white-space:pre-wrap;">${b.content}</div>
                   </div>`;
      });
  }
...
async function calculateDashboardStats() {
  ...
  if (filter && filter.activated_at) {
    ...
    if (health >= 60) {
        ...
        if(badgeText) badgeText.innerText = '請即刻更換'; 
...
function getSnapshotGPS() {
  let lastCity = localStorage.getItem('hp_last_city') || (currentUser ? currentUser.city : '台北市'); 
...
async function init() {
  ...
  if (data) {
    ...
    setElText('ui-home-city', data.city || '台北市');
